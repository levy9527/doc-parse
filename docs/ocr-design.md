...
| `PDF_WORKERS` | `2` | 页级并行进程数（1=串行）。并行度 ≈ 核数 ÷ 单页线程数；约束 `PDF_WORKERS × OCR_INTRA_THREADS ≤ 核数` |
...
实现：`parse_pdf` 先算出每页「是否 OCR」，再用多进程池按页并行（Linux `fork`、macOS `spawn`），
结果按页码保序拼接；`PDF_WORKERS` 控制进程数（默认 2，1=串行）。测试覆盖并行/串行输出一致性
（含一个 2 页纯图片 PDF 的真实并行 OCR 用例）。

原理与约束：**并行度 ≈ 可见核数 ÷ 单页线程数**。
- Mac 12 核：单页 OCR 已吃满 12 核（CPU≈1300%）→ 页级并行无空间，建议 `PDF_WORKERS=1`；
- 服务器 96 核：单页只用 ~16 线程 → 有大量空闲核，可并行 2~4 页。
- 约束：`PDF_WORKERS × OCR_INTRA_THREADS ≤ 核数`，否则回到超额订阅；
- 代价：每进程各加载一套 ONNX 模型（内存 ×N）、进程启动开销（池化摊薄）。

**服务器实测（教材 120 页，全 OCR；4 次运行输出完全一致，82,263 字 / 120 页）**：

| 配置 | 耗时 | 相对基准 |
|---|---|---|
| 旧镜像（poppler + auto 线程） | 424.2s | 基准 |
| 新镜像（pdfium + auto 线程） | 400.1s | 快 5.7% |
| 新镜像（pdfium + `OCR_INTRA_THREADS=16`，串行） | 268.6s | 快 1.58 倍 |
| 新镜像（pdfium + `OCR_INTRA_THREADS=16` + `PDF_WORKERS=2`） | 144.4s | 快 2.94 倍 |
| 新镜像（pdfium + `OCR_INTRA_THREADS=16` + `PDF_WORKERS=4`） | **72.5s** | **快 5.85 倍** |
| 同上但 `PDF_WORKERS=6` | 71.6s | 快 5.92 倍（较 4 进程仅快 0.9s） |
| 同上但 `PDF_WORKERS=8` | 74.4s | 快 5.70 倍（转慢） |

> `PDF_WORKERS` 在 96 核服务器上 2→4 进程**近乎完美线性**（144.4s → 72.5s，正好 2 倍）；
> 但 **4 进程即平台期**：6 进程 71.6s（仅快 0.9s）、8 进程 74.4s（转慢）。
> 理论上限 = 核数 ÷ 单页线程数 = 96 ÷ 16 = 6，但**生产取 4 留余量**给其他容器，
> 收益差 1% 却少占 32 个核。
> 同配置在 **Mac 12 核上是负收益**（ppt.pdf 串行 6.94s vs 并行 8.52s）：单页已吃满 12 核，
> `2 × 8` 线程变成超额订阅 → 所以 **12 核机器建议 `PDF_WORKERS=1`**。
